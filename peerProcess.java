...
class peerProcess
{
    ...
    {
        collectedPieces++; if (collectedPieces == totalPieces)
    {
        hasFile = true;
    }
    }
    ...
    {
        this.peerId = peerId; logger = new Logger(peerId);

        try
        {
            //https://www.educative.io/edpresso/reading-the-nth-line-from-a-file-in-java
            Path tempFile = Paths.get("Common.cfg"); List<String> fileLines = Files.readAllLines(tempFile);
            String fileSizeString = fileLines.get(4); String pieceSizeString = fileLines.get(5);
            String[] fileSizes = fileSizeString.split(" "); String[] pieceSizes = pieceSizeString.split(" ");

            fileSize = Integer.parseInt(fileSizes[1]); pieceSize = Integer.parseInt(pieceSizes[1]);
        }
        catch (Exception e)
        {

        } totalPieces = (int) ceil((double) fileSize / pieceSize); bitfield.setSize(totalPieces);
        FileObject = new FileHandling(this.peerId, totalPieces, pieceSize); hasFile = false;
    ...
    {  // searches through a list of peers to find the one with the matching ID
        for (RemotePeerInfo rfi : peerInfoVector)
        {        // iterates over the set of rfi peers
            if (rfi.getPeerId() == peerId)
            {               // if the rfi's peerID matches, then return the rfi handle
                return rfi;
            }
        } return null;
    }
    ...
    public void ReadCommongConfig(int peerId)
    {
        ...
        try
        {
            ...
            st = in.readLine(); String[] tokens = st.split("\\s+");
            this.numberOfPreferredNeighbors = Integer.parseInt(tokens[1]);

            st = in.readLine(); tokens = st.split("\\s+"); this.unchokingInterval = Integer.parseInt(tokens[1]);

            st = in.readLine(); tokens = st.split("\\s+");
            this.optimisticUnchokingInterval = Integer.parseInt(tokens[1]);

            st = in.readLine(); tokens = st.split("\\s+"); this.fileName = tokens[1];

            st = in.readLine(); tokens = st.split("\\s+"); this.fileSize = Integer.parseInt(tokens[1]);

            st = in.readLine(); tokens = st.split("\\s+"); this.pieceSize = Integer.parseInt(tokens[1]);
    ...
    private static int GetProcessId(String[] args)
    {
        if (args.length == 0)
        {
            System.out.println("Process number must be provided."); return -1;
        } String peerIdString = args[0];
        // if peerIdString is not a positive integer
        int peerId = 0; try
    {
        peerId = Integer.parseInt(peerIdString); assert peerId >= 0;
    }
    catch (Exception e)
    {
    ...
    public void startTCPConnection(StartRemotePeers srp, int peerId) throws Exception
    {
        ...
        if (peerId != 1001)
        { // if client
            System.out.println("Attempting to connect as a client to the port..."); client = new Client(this);
            client.setPeerID(peerId); client.run();
        } else
        { // if server
            System.out.println("Starting a listener at the port and try to handshake with other processes...");
            server = new Server(this); server.startServer();
        }
    }
    ...
    {
        int min = 4; int max = peerInfoVector.size();
        int randomPeerIndex = (int) Math.floor(Math.random() * (max - min + 1) + min);
        if (randomPeerIndex > peerInfoVector.size() - 1) randomPeerIndex = peerInfoVector.size() - 1;
    ...
    private void sortPeerInfoVector()
    {
        Collections.sort(peerInfoVector, (o1, o2) ->
        {
            ...
            {
                Random chooser = new Random(); if (chooser.nextInt(3) == 1)
            {
                return -1;
            } else return 1;
    ...
    {
        int peerId = GetProcessId(args); if (peerId == -1)
    {
        return;
    } peerProcess pp = new peerProcess(peerId); StartRemotePeers srp = new StartRemotePeers(pp);
        //srp.Start(peerId);
