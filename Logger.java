...
class Logger {
    ...
    public void onChangeOfOptimisticallyUnchokedNeighbor(String time, String optimisticallyUnchockedNeighbor_ID) {
        log(time + ": Peer " + this.peerID + "has the optimistically unchoked neighbor " + optimisticallyUnchockedNeighbor_ID + ".");
    } 
    ...
    public void onDownloadingAPiece(String time, String peerID2, int piece_index, int number_of_pieces) {
        log(time + ": Peer " + this.peerID + " has downloaded the piece " + String.valueOf(piece_index) +
                " from " + peerID2 + ". Now the number of pieces it has is " + String.valueOf(number_of_pieces) + ".");
    }
